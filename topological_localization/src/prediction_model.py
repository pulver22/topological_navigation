...
class PredictionModel:
    ...
    def _ctmc(tmap, node, speed, time):
        ...
        same_direction_mask = np.any(speed * self.node_diffs2D[self.connected_nodes] >= 0, axis=1)
        pos_connected_nodes = self.connected_nodes[same_direction_mask]
        neg_connected_nodes = self.connected_nodes[~same_direction_mask]
        ...
        # project speed vector on the edges toward successive nodes
        diffs_norm = np.dot(
            self.node_diffs2D[pos_connected_nodes], self.node_diffs2D[pos_connected_nodes].transpose()).diagonal()
        speed_proj = (np.dot(self.node_diffs2D[pos_connected_nodes], speed) /
                        diffs_norm).reshape((-1, 1)) * self.node_diffs2D[pos_connected_nodes]
        speed_proj = np.sqrt(
            np.dot(speed_proj, speed_proj.transpose()).diagonal())
        ...
        if lambda_p <= 0:
            _lambda_p = - np.log(0.5) * speed_proj / \
                self.node_distances[pos_connected_nodes]
        else:
            _lambda_p = np.ones(speed_proj.shape) * lambda_p
        p_move = 1. - np.exp(- time * _lambda_p)
    ...
    def predict(node, time=0, speed=0):
        if self.pred_type == PredictionModel.CTMC:
            return self._ctmc(node, time, speed)
        else:
